import { InventoryGroups, InventorySections } from '../data/inventory-data';
import type { CheckedState, InventoryItem } from '../types';

export class InventoryManager {
  private container: HTMLElement | null;
  private inventoryFilter: 'all' | 'E' | 'O' = 'all';

  constructor() {
    this.container = document.getElementById('inventory-sections');
    this.init();
  }
  ...
  private getCheckedState(section: string): CheckedState {
    return JSON.parse(localStorage.getItem(`checked_${section}`) || '[]');
  }

  private setCheckedState(section: string, arr: CheckedState): void {
  ...
  private renderInventoryFilterDropdown(): void {
    ...
    if (!filterDiv) {
      ...
      select.addEventListener('change', (e) => {
        this.inventoryFilter = (e.target as HTMLSelectElement).value as 'all' | 'E' | 'O';
        this.renderInventoryAccordions();
      });
  ...
  private renderSection(
    ...
    let checkedArr = this.getCheckedState(sectionKey);
    if (!Array.isArray(checkedArr) || checkedArr.length !== items.length) {
      checkedArr = Array(items.length).fill(false);
    }
    
    const filteredItems = items.filter(item => this.inventoryFilter === 'all' || item.type === this.inventoryFilter);

    filteredItems.forEach((item, originalIndex) => {
      const originalItemIndex = items.findIndex(i => i.text === item.text && i.type === item.type);
      ...
      checkbox.checked = checkedArr[originalItemIndex];
      
      checkbox.addEventListener('change', () => {
        checkedArr[originalItemIndex] = checkbox.checked;
        this.setCheckedState(sectionKey, checkedArr);
      });
  ...
  private renderInventoryAccordions(): void {
    ...
    const filterDiv = document.getElementById('inventory-filter-div');
