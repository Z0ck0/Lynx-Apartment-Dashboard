import { CleaningGroups, CleaningSections } from '../data/checklist-data';
...
export class ChecklistManager {
  ...
  constructor() {
    this.container = document.getElementById('checklist-sections');
    this.init();
  }
  ...
  private renderCleaningAccordions(): void {
    ...
    if (expandAllBtn) {
      expandAllBtn.addEventListener('click', () => this.toggleAllSections(true));
    }
    if (collapseAllBtn) {
      collapseAllBtn.addEventListener('click', () => this.toggleAllSections(false));
    }
    ...
    CleaningGroups.forEach(groupObj => {
      ...
      groupObj.sections.forEach(section => {
        ...
        const checkedArr = this.getCheckedState(section);
          ...
          this.renderSectionWithProgress(
            ...
            () => {
              ...
              groupObj.sections.forEach(s => {
                ...
                const checkedArr = this.getCheckedState(s);
                if (Array.isArray(checkedArr)) {
                  newCompleted += checkedArr.filter(Boolean).length;
                }
              });
              const statsElement = header.querySelector('.section-stats');
              if (statsElement) {
