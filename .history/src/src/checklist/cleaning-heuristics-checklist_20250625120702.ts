import { CleaningHeuristics, QuickHeuristics, CommonOversights, CleaningSequence } from "../data/cleaning-heuristics";
...
export class CleaningHeuristicsChecklist {
      ...
      <div class="heuristics-container">
        ...
        <div class="heuristics-grid">
          <div class="heuristic-section">
            ...
            ${this.renderHeuristicsByPriority("critical")}
          </div>
  ...
  private renderHeuristicsByPriority(priority: "critical" | "high" | "medium" | "low") {
    const heuristics = CleaningHeuristics.filter(h => h.priority === priority);
    ...
    return heuristics.map(heuristic => `
      <div class="heuristic-card ${priority}">
        ...
        <div class="heuristic-tips">
          <strong>Tips:</strong>
          <ul>
            ${heuristic.tips.map(tip => `<li>${tip}</li>`).join('')}
          </ul>
        </div>
      </div>
    `).join('');
  ...
  private renderQuickHeuristics() {
    return Object.entries(QuickHeuristics).map(([scenario, items]) => `
  ...
  private renderCommonOversights() {
    return Object.entries(CommonOversights).map(([area, items]) => `
  ...
  private renderCleaningSequence() {
    return `
      <div class="cleaning-sequence">
        <ol>
          ${CleaningSequence.map(step => `<li>${step}</li>`).join('')}
        </ol>
      </div>
    `;
  }
  ...
  private attachEventListeners() {
    ...
    this.container.querySelectorAll('input[data-heuristic]').forEach(checkbox => {
      checkbox.addEventListener('change', (e) => {
        const target = e.target as HTMLInputElement;
        const heuristic = target.dataset.heuristic!;
        const item = target.dataset.item!;
        
        if (target.checked) {
          this.validator.markTaskCompleted(item);
        }
      });
    });

    // Handle oversight checkboxes
    this.container.querySelectorAll('input[data-area]').forEach(checkbox => {
      checkbox.addEventListener('change', (e) => {
        const target = e.target as HTMLInputElement;
        ...
        if (target.checked) {
          this.validator.markTaskCompleted(item, area);
        }
      });
    });

    // Handle validation button
    const validateBtn = this.container.querySelector('#validate-cleaning');
    if (validateBtn) {
  ...
  private showValidationResult() {
    const result = this.validator.validateCleaning();
    const resultDiv = this.container.querySelector('#validation-result');
    
    if (resultDiv) {
      resultDiv.innerHTML = `
        <div class="validation-result">
          <div class="score ${result.score >= 80 ? 'good' : result.score >= 60 ? 'warning' : 'poor'}">
